...
// const socket = io("http://10.99.232.91:3001", {
...
//     //         breathing: data.breathing_status || "--",  // repurpose → ECG Stable HR
//     //         ecg:      data.ecg_hr    || "--",       // repurpose → ECG Fast HR
//     //         eegAlpha: data.alert     || "--",       // repurpose → Alert status
//     //         eegBeta:  "--",
//     //         eegGamma: "--",
//     //         breathing: data.status    || "--",      // "NORMAL" / "ABNORMAL!" / "CLIPPING"
//     //         eegAlpha: data.alpha      || "--",
//     //         eegBeta:  data.beta       || "--",
//     //         eegGamma: data.gamma      || "--",
//     //       });
...
//         <VitalCard icon={Heart}          label="SparkFun HR"     value={vitals.bpm}       unit="BPM"  color="rose"    />
//         <VitalCard icon={Gauge}          label="Presence"        value={vitals.presence}  unit=""     color="amber"   />
//         <VitalCard icon={Activity}       label="ECG Fast HR"     value={vitals.ecg}       unit="BPM"  color="red"     />
//         <VitalCard icon={Waves}          label="ECG Stable HR"   value={vitals.breathing} unit=""  color="emerald" />
//         <VitalCard icon={AlertTriangle}  label="Alert"           value={vitals.eegAlpha}  unit=""     color="indigo"  />
//         <VitalCard icon={Activity} label="EEG RMS"    value={vitals.ecg}      unit=""  color="red"     />
//         <VitalCard icon={Wind}     label="EEG Status" value={vitals.breathing} unit=""  color="emerald" />
//         <VitalCard icon={Wind}           label="Breathing"       value={vitals.breathing} unit="/min" color="sky" />
//       </div>
...
export default function LiveMonitor() {
  ...
  useEffect(() => {
    const socket = io("http://10.99.232.91:3001", { transports: ["websocket"] });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-teal-50 to-cyan-50">
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" onClick={() => navigate("/mode")}>
              <ArrowLeft className="w-4 h-4 mr-2" /> Back
            </Button>
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 bg-gradient-to-br ${currentMode.gradient} rounded-lg flex items-center justify-center`}>
