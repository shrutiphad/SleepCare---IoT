...
// Keep for longer if you want more history.
ReadingSchema.index({ timestamp: 1 }, { expireAfterSeconds: 86400 });
...
function deriveSleepStage(alpha = 0, beta = 0, gamma = 0) {
  if (alpha > 8 && beta > gamma) return "Awake";
  if (alpha > beta && alpha > gamma) return "Relaxed / Drowsy";
  if (alpha < 5 && beta < 5) return "Deep Sleep";
  if (alpha > 0 || beta > 0) return "Light Sleep";
  return "--";
}
...
function computeRisk(data) {
  ...
  if (data.spo2 > 0) {
    if (data.spo2 < 85) {
      score += 50;
      reasons.push(`Critical SpO₂: ${data.spo2}%`);
    } else if (data.spo2 < 90) {
      score += 35;
      reasons.push(`Dangerous SpO₂: ${data.spo2}%`);
    } else if (data.spo2 < 94) {
      score += 20;
      reasons.push(`Low SpO₂: ${data.spo2}%`);
    }
  }
  ...
  if (data.heart_rate > 0) {
    if (data.heart_rate > 120) {
      score += 25;
      reasons.push(`Tachycardia: ${data.heart_rate} BPM`);
    } else if (data.heart_rate < 50) {
      score += 25;
      reasons.push(`Bradycardia: ${data.heart_rate} BPM`);
    }
  }

  const cnnRisk = { AFib: 30, Tachycardia: 20, Bradycardia: 20 };
  if (cnnRisk[data.cnn_label]) {
    score += cnnRisk[data.cnn_label];
    reasons.push(`CNN: ${data.cnn_label}`);
  }

  if (data.breathing_status?.toLowerCase().includes("apnea")) {
    score += 40;
    reasons.push("Apnea event");
  }

  if ((data.rms || 0) > 400) {
    score += 15;
    reasons.push(`High EEG RMS: ${data.rms}`);
  }

  const final = Math.min(100, score);
  return {
    score: final,
    level: final >= 50 ? "critical" : final >= 20 ? "warning" : "normal",
    reasons,
  };
}
...
class ECGRingBuffer {
  ...
  snapshot() {
    if (this.size < this.capacity) {
      return Array.from(this.buf.slice(0, this.size));
    }
    return Array.from(this.buf.slice(this.head)).concat(
      Array.from(this.buf.slice(0, this.head))
    );
  }
...
async function runCnnClassifier(samples) {
  try {
    ...
    lastCnnLabel = r.label || "Normal";
    lastCnnConfidence = r.confidence ?? 0;

    io.emit("ecg-classification", {
      label: lastCnnLabel,
      confidence: lastCnnConfidence,
    });
...
io.on("connection", (socket) => {
  ...
  socket.emit("ecg-classification", {
    label: lastCnnLabel,
    confidence: lastCnnConfidence,
  });
...
async function pushReadingToRag(data) {
  try {
    await fetch(`${RAG_SERVER}/store-reading`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
  } catch (err) {
    log.warn(`[RAG STORE] ${err.message}`);
  }
}
...
app.post("/sensor-data", async (req, res) => {
  ...
  if (!incoming || ![1, 2, 3, 4].includes(incoming.mode)) {
  ...
  const data = {
    ...
    sleep_stage:
      incoming.mode === 3
        ...
        : incoming.sleep_stage ?? null,
  };

  const { score, level, reasons } = computeRisk({
    ...data,
  ...
  if (data.mode === 2 && typeof data.ecg_hr === "number") {
    ...
    if (shouldClassify) {
      ecgBuffer.resetTrigger();
      void runCnnClassifier(ecgBuffer.snapshot());
    }
  }
...
app.get("/history/normal", async (req, res) => {
  try {
    res.json(
      await Reading.aggregate(
        historyPipeline(1, Math.min(parseInt(req.query.minutes) || 5, 10))
      )
    );
  } catch (e) {
...
app.get("/history/heart", async (req, res) => {
  try {
    res.json(
      await Reading.aggregate(
        historyPipeline(2, Math.min(parseInt(req.query.minutes) || 5, 10))
      )
    );
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.get("/history/brain", async (req, res) => {
  try {
    res.json(
      await Reading.aggregate(
        historyPipeline(3, Math.min(parseInt(req.query.minutes) || 5, 10))
      )
    );
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.get("/history/breathing", async (req, res) => {
  try {
    res.json(
      await Reading.aggregate(
        historyPipeline(4, Math.min(parseInt(req.query.minutes) || 5, 10))
      )
    );
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.post("/query", async (req, res) => {
  ...
  try {
    const r = await fetch(`${RAG_SERVER}/query`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
app.get("/db-stats", async (_req, res) => {
  try {
    ...
    const byMode = await Reading.aggregate([
      { $group: { _id: "$mode", count: { $sum: 1 }, latest: { $max: "$timestamp" } } },
    ]);
...
app.get("/api/download/brain", async (_req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="brain_report.xlsx"');
...
app.get("/api/download/heart", async (_req, res) => {
  try {
    const workbook = new ExcelJS.Workbook();
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="heart_report.xlsx"');
...
app.get("/api/download/normal", async (_req, res) => {
  try {
    const workbook = new ExcelJS.Workbook();
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="normal_report.xlsx"');
...
app.get("/api/download/breathing", async (_req, res) => {
  try {
    const workbook = new ExcelJS.Workbook();
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="breathing_report.xlsx"');
